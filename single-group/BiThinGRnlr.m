function [parT,parC] = BiThinGRnlr(PAR,parTi,emc1,rii1,dlexp,PLexp,N1)
...
%
lb = [eps eps eps eps Gmmin Gcmin eps];
ub = [1e4 c2mmax 1e4 c2cmax Gmmax Gcmax 1e2];
%
...
%
ldM1 = sqrt(ltM1.^2*sin(PAR(5))^2+lz1.^2*cos(PAR(5))^2);	% diagonal stretch media, with PAR(6) = alpha
ldA1 = sqrt(ltA1.^2*sin(PAR(5))^2+lz1.^2*cos(PAR(5))^2);	% diagonal stretch adventitia, with PAR(6) = alpha
%
...
% 	%
% 	ldM2 = sqrt(ltM2.^2*sin(PAR(5))^2+lz2.^2*cos(PAR(5))^2);	% diagonal stretch media, with PAR(6) = alpha
% 	ldA2 = sqrt(ltA2.^2*sin(PAR(5))^2+lz2.^2*cos(PAR(5))^2);	% diagonal stretch adventitia, with PAR(6) = alpha
% 	%
...
while ([I - II ~= zeros(size(I)); length(I) > 2])
	...
	%
	parT = lsqcurvefit(@(parT,dl) PreForGRT(parT,dl,PAR,emc1,rii1,NI1),parT,dlexp(I,:),PLexp(I,:),lb,ub,options);
...
ub = parT(1:4);
%
parC = lsqcurvefit(@(parC,dl) PreForGRTC(parC,dl,PAR,parT,emc1,rii1,N1),parC,dlexp,PLexp,lb,ub,options);
